#ifndef LD06Driver_h
#define LD06Driver_h

#define PRINT(a) \
    do {         \
    } while (false)
//_serialPortPC.print(#a);
//_serialPortPC.print(" = ");
//_serialPortPC.println(a);
//}
// while (false)

// #include "main.h"
#include <Arduino.h>
#include <LinkedList.h>

#define _serialPortLidar Serial2
#define _serialPortPC Serial

// note: 12 data *3 Byte
const int DATA_BYTE = 36;
// note: 1(Start)+2(Datalen)+2(Speed)+2(SAngle)+36(DataByte)+2(EAngle)+2(TimeStamp)+1(CRC)
const int TOTAL_DATA_BYTE = 48;

const byte test[47] = {0x54, 0x2C, 0x68, 0x08, 0xAB, 0x7E, 0xE0, 0x00, 0xE4, 0xDC, 0x00, 0xE2, 0xD9, 0x00, 0xE5, 0xD5,
                       0x00, 0xE3, 0xD3, 0x00, 0xE4, 0xD0, 0x00, 0xE9, 0xCD, 0x00, 0xE4, 0xCA, 0x00, 0xE2, 0xC7, 0x00,
                       0xE9, 0xC5, 0x00, 0xE5, 0xC2, 0x00, 0xE5, 0xC0, 0x00, 0xE5, 0xBE, 0x82, 0x3A, 0x1A, 0x50};

const byte CrcTable[256] = {
    };

class PointData {
   public:
    double angle = 0;
    int distance = 0;
    int confidence = 0;
    int timestamp = 0;
};

class LidarPacket {
   public:
    byte header;
    int dataLength;
    int radarSpeed;
    int startAngle;

    // data bytes(distance and confidence data in raw format)
    LinkedList<byte> data;
    int endAngle;
    int timestamp;
    byte crcCheck;

    // Raw packet bytes received (used to calculate CRC)
    LinkedList<byte> packetRaw;

   public:
    void Initialisation() {
        header = 0x00;
        dataLength = 0;
        radarSpeed = 0;
        startAngle = 0;
        data.clear();
        endAngle = 0;
        timestamp = 0;
        crcCheck = 0x00;
        packetRaw.clear();
    }

    byte CalCRC8()  // string packet, byte len)
    {
        // convert buffer queue to array
        // byte[] packet = packetRaw.ToArray();

        // debug output
        // var hexString = BitConverter.ToString(packet);
        // Console.WriteLine("Packet: " + hexString);
        // Console.WriteLine("Packet len: " + packet.Length);
        byte crc = 0;

        for (int i = 0; i < packetRaw.size() - 1; i++) {
            crc = CrcTable[(crc ^ packetRaw[i]) & 0xff];
        }

        return crc;
    }
};

class LD06Driver {
   public:
    void Initialisation();
    void ReadLidarData();
};

#endif