...
uint32_t ledcSetup(uint8_t chan, uint32_t freq, uint8_t bit_num)
{
    ...
    */
    ...
    return 0;// ledc_get_freq(group, timer);
}
...
uint32_t ledcRead(uint8_t chan)
{
    ...
    uint8_t group=(chan/8), channel=(chan%8);
    return 0;// ledc_get_duty(group, channel);
}
...
uint32_t ledcWriteTone(uint8_t chan, uint32_t freq)
{
    ...
    */
    ...
    uint32_t res_freq = 0;//ledc_get_freq(group,timer);
...
void ledcAttachPin(uint8_t pin, uint8_t chan)
{
    ...
    uint32_t duty = 0;//ledc_get_duty(group,channel);
